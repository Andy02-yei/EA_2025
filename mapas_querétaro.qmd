...
Este conjunto de datos para el estado de Querétaro incluye: - Límites estatales (archivos `14ent.*`) - Límites municipales (archivos `14mun.*`) - Otras divisiones territoriales
...
Primero, leemos el archivo shapefile para Querétaro:

```{r entidades}
# Nota: Asegúrate de que todos los archivos (shp, shx, dbf, prj, cpg) estén en el mismo directorio: data

Querétaro_estado <- st_read("data_q/22ent.shp", options = "ENCODING=LATIN1", quiet = TRUE)
```

### Visualización del estado de Querétaro

```{r mapa_Querétaro_estado}
# Crear mapa básico de Querétaro
mapa_Querétaro <- ggplot(Querétaro_estado) +
...
```{r municipios}
municipios_Querétaro <- st_read("data_q/22mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)

# Explorar los datos
cat("Número de municipios en Querétaro:", nrow(municipios_Querétaro), "\n")
cat("Primeros 10 municipios:\n")
print(municipios_Querétaro$NOMGEO[1:10])
```

### Visualización de municipios

```{r mapa_municipios}
# Crear mapa de municipios de Querétaro
mapa_municipios <- ggplot(municipios_Querétaro) +
  geom_sf(fill = "lightgreen", color = "white", size = 0.3) +
  labs(
    title = "Municipios de Querétaro",
    subtitle = paste("Total de municipios:", nrow(municipios_Querétaro)),
    caption = "Fuente: INEGI - Marco Geoestadístico"
  ) +
  theme_void() +
  theme(
...
municipios_importantes <- c("Tequisquiapan", "Tolimán", "Landa de Matamoros", "Corregidora", "Arroyo Seco", "El Marqués")

# Crear una variable para destacar estos municipios
municipios_Querétaro <- municipios_Querétaro %>%
  mutate(
...
mapa_destacados <- ggplot(municipios_Querétaro) +
  ...
  labs(
    title = "Municipios Importantes de Querétaro",
...
municipios_grandes <- municipios_Querétaro |> 
  st_drop_geometry() |> 
  arrange(desc(area_km2)) |> 
  select(NOMGEO, area_km2) |> 
  head(10)

print("Los 10 municipios más grandes de Querétaro:")
...
mapa_areas <- ggplot(municipios_Querétaro) +
  ...
  labs(
    title = "Área de los Municipios de Querétaro",
