...
```{r}
comparar_geometricas_bneg <- function(r, p, n_sim) {
  ...
  # Tabla
  print(kable(stats_comparacion, digits = 4,
              caption = paste("Verificación de la proposición (r =", r, ", p =", p, ", n_sim =", n_sim, ")")))
  ...
  df_comparacion <- data.frame(
    ...
    Método = rep(c("Suma de Geométricas", "Binomial Negativa"), each = n_sim)
  )
  
  # Gráfico comparativo
  plot_comparativo <- ggplot(df_comparacion, aes(x = Valor, fill = Método)) +
    geom_histogram(alpha = 0.6, position = "identity", bins = 30, color = "black") +
    geom_vline(xintercept = media_teorica, linetype = "dashed", color = "#8B4513", linewidth = 1.2) +
    annotate("text", x = media_teorica, y = Inf, 
             label = paste("Media teórica =", round(media_teorica, 2)), 
             color = "#8B4513", vjust = 2, size = 4.5, fontface = "bold") +
    scale_fill_manual(values = c("Suma de Geométricas" = "#FF9800", 
                                  "Binomial Negativa" = "#2196F3")) +
    labs(
      title = "Comparación: Suma de Geométricas vs Binomial Negativa",
      subtitle = paste("r =", r, ", p =", p, ", n_sim =", format(n_sim, big.mark = ",")),
      x = "Número de fracasos",
      y = "Frecuencia",
    ) +
    theme_minimal() +
  ...
  return(plot_comparativo)
}

# Ejemplos

cat("Ejemplo 1: r = 4, p = 0.3, n_sim = 10000\n")
grafico1 <- comparar_geometricas_bneg(r = 4, p = 0.3, n_sim = 10000)
print(grafico1)

cat("\n\nEjemplo 2: r = 10, p = 0.5, n_sim = 5000\n")
grafico2 <- comparar_geometricas_bneg(r = 10, p = 0.5, n_sim = 5000)
print(grafico2)
```
:::
...
cat("Jugador de baloncesto\n")
cat("Efectividad:", p_exito * 100, "%\n")
cat("Meta:", r_tiros, "tiros exitosos consecutivos\n\n")

# a) Modelar el problema usando suma de geométricas
cat("a) Modelado del problema:\n")
cat("   X = número de tiros fallados antes de conseguir", r_tiros, "tiros exitosos\n")
cat("   X = Y₁ + Y₂ + ... + Y₈\n")
cat("   donde cada Yᵢ ~ Geométrica(p =", p_exito, ")\n")
cat("   Por la proposición: X ~ Binomial Negativa(r =", r_tiros, ", p =", p_exito, ")\n\n")

# b) Número esperado de tiros fallados
tiros_fallados_esperados <- r_tiros * (1 - p_exito) / p_exito
varianza_fallados <- r_tiros * (1 - p_exito) / (p_exito^2)
desv_std_fallados <- sqrt(varianza_fallados)

cat("b) Número esperado de tiros fallados:\n")
cat("   Media (teórica):", round(tiros_fallados_esperados, 4), "\n")
cat("   Varianza:", round(varianza_fallados, 4), "\n")
cat("   Desviación estándar:", round(desv_std_fallados, 4), "\n")
cat("   Se esperan aproximadamente", round(tiros_fallados_esperados, 2), 
    "tiros fallados antes de lograr 8 exitosos\n\n")

tiros_totales_esperados <- r_tiros + tiros_fallados_esperados
cat("   Tiros totales esperados:", round(tiros_totales_esperados, 2), 
    "(", r_tiros, "exitosos +", round(tiros_fallados_esperados, 2), "fallados )\n\n")

# c) Simulación de 1000 sesiones de entrenamiento
n_sim <- 1000
set.seed(42)

# Suma de geométricas
simulacion_tiros <- replicate(n_sim, {
  sum(rgeom(r_tiros, prob = p_exito))
})

# Estadísticas de la simulación
media_sim <- mean(simulacion_tiros)
desv_std_sim <- sd(simulacion_tiros)
varianza_sim <- var(simulacion_tiros)

cat("c) Simulación de", n_sim, "sesiones de entrenamiento:\n")
cat("   Media simulada:", round(media_sim, 4), "\n")
cat("   Desv. Std. simulada:", round(desv_std_sim, 4), "\n")
cat("   Varianza simulada:", round(varianza_sim, 4), "\n\n")

cat("   Comparación con valores teóricos:\n")
cat("     Media teórica:", round(tiros_fallados_esperados, 4), "\n")
cat("     Media simulada:", round(media_sim, 4), "\n")
cat("     Diferencia:", round(abs(media_sim - tiros_fallados_esperados), 4), "\n\n")

cat("     Desv. Std. teórica:", round(desv_std_fallados, 4), "\n")
cat("     Desv. Std. simulada:", round(desv_std_sim, 4), "\n")
cat("     Diferencia:", round(abs(desv_std_sim - desv_std_fallados), 4), "\n\n")

# Tabla comparativa
tabla_comparacion <- data.frame(
  Método = c("Teórico", "Simulación"),
  Media = c(tiros_fallados_esperados, media_sim),
  Varianza = c(varianza_fallados, varianza_sim),
  Desv_Std = c(desv_std_fallados, desv_std_sim)
)

kable(tabla_comparacion, 
      digits = 4,
      caption = paste("Comparación: Valores teóricos vs Simulación (", n_sim, "sesiones)"),
      col.names = c("Método", "Media", "Varianza", "Desv. Std."))


# Histograma con distribución teórica
df_simulacion <- data.frame(
  Tiros_Fallados = simulacion_tiros
)

# Valores teóricos para comparar
x_teorico <- 0:max(simulacion_tiros)
y_teorico <- dnbinom(x_teorico, size = r_tiros, prob = p_exito) * n_sim
...
plot_comparativo <- ggplot() +
  geom_histogram(data = df_simulacion, aes(x = Tiros_Fallados, y = ..count..), 
                 bins = 25, fill = "#FF8C00", alpha = 0.7, color = "black") +
  geom_line(data = df_teorico, aes(x = x, y = y), 
            color = "red", size = 1.2, linetype = "dashed") +
  geom_vline(xintercept = tiros_fallados_esperados, linetype = "dashed", 
             color = "#458B00", size = 1) +
  annotate("text", x = tiros_fallados_esperados, y = Inf, 
           label = paste("Media teórica =", round(tiros_fallados_esperados, 2)), 
           color = "#458B00", vjust = 2, size = 4.5, fontface = "bold") +
  labs(
    title = "Simulación: Suma de 8 variables Geométricas",
    subtitle = paste("p =", p_exito, ", r =", r_tiros, ", n_sim =", format(n_sim, big.mark = ",")),
...
# Parámetros teóricos
pruebas_fallidas_esperadas <- r_pruebas * (1 - p_exito) / p_exito
varianza_fallidas <- r_pruebas * (1 - p_exito) / (p_exito^2)
desv_std_fallidas <- sqrt(varianza_fallidas)

cat("Parámetros estadísticos\n")
cat("Media:", round(pruebas_fallidas_esperadas, 4), "\n")
cat("Varianza:", round(varianza_fallidas, 4), "\n")
cat("Desviación estándar:", round(desv_std_fallidas, 4), "\n\n")

# a) Probabilidad de tener exactamente 12 pruebas fallidas
prob_12_fallidas <- dnbinom(12, size = r_pruebas, prob = p_exito)

cat("a) Probabilidad de tener exactamente 12 pruebas fallidas:\n")
cat("   P(X = 12) =", round(prob_12_fallidas, 6), "\n")
cat("   P(X = 12) =", sprintf("%.4f%%", prob_12_fallidas * 100), "\n")

# b) Probabilidad de necesitar más de 20 pruebas fallidas
prob_mas_20_fallidas <- 1 - pnbinom(20, size = r_pruebas, prob = p_exito)

cat("b) Probabilidad de necesitar más de 20 pruebas fallidas:\n")
cat("   P(X > 20) =", round(prob_mas_20_fallidas, 6), "\n")
cat("   P(X > 20) =", sprintf("%.4f%%", prob_mas_20_fallidas * 100), "\n")

# c) Costo esperado en pruebas fallidas
costo_esperado_fallidas <- pruebas_fallidas_esperadas * costo_prueba

cat("c) Costo esperado en pruebas fallidas:\n")
cat("   Número esperado de pruebas fallidas:", round(pruebas_fallidas_esperadas, 2), "\n")
cat("   Costo esperado: $", format(round(costo_esperado_fallidas, 2), big.mark = ","), "\n")

# Visualización
x_values <- 0:35
prob_values <- dnbinom(x_values, size = r_pruebas, prob = p_exito)

df_distribucion <- data.frame(
  Pruebas_Fallidas = x_values,
  Probabilidad = prob_values
) %>%
  mutate(
    Color = case_when(
      Pruebas_Fallidas == 12 ~ "Exactamente 12",
      Pruebas_Fallidas > 20 ~ "Más de 20",
      TRUE ~ "Otros"
    )
  )

plot_distribucion <- ggplot(df_distribucion, aes(x = Pruebas_Fallidas, y = Probabilidad, fill = Color)) +
  geom_col(color = "black", width = 0.8) +
  scale_fill_manual(values = c("Exactamente 12" = "#FF9800", 
                                "Más de 20" = "#F44336",
                                "Otros" = "#B0E0E6"),
                    name = "Categoría") +
  geom_vline(xintercept = pruebas_fallidas_esperadas, linetype = "dashed", 
             color = "#2F4F4F", size = 1.2) +
  annotate("text", x = pruebas_fallidas_esperadas, y = max(prob_values) * 0.9, 
           label = paste("Media =", round(pruebas_fallidas_esperadas, 2)), 
           color = "#2F4F4F", size = 4.5, fontface = "bold") +
  labs(
    title = "Distribución Binomial Negativa - Pruebas de laboratorio",
    subtitle = paste("r =", r_pruebas, ", p =", p_exito, "| Naranja: X = 12 | Rojo: X > 20"),
