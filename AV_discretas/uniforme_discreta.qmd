...
```{r}
cat("Urna con bolas numeradas\n")

valores_urna <- 1:8
n_valores_urna <- length(valores_urna)

prob_uniforme <- 1 / n_valores_urna

cat("Valores posibles:", paste(valores_urna, collapse = ", "), "\n")
cat("Probabilidad de cada valor:", prob_uniforme, "\n")

numeros_pares <- valores_urna[valores_urna %% 2 == 0]
n_pares <- length(numeros_pares)

prob_par <- n_pares / n_valores_urna

cat("Números pares:", paste(numeros_pares, collapse = ", "), "\n")
cat("Cantidad de números pares:", n_pares, "\n")
cat("Probabilidad de obtener número par:", prob_par, "\n")
cat("Probabilidad de obtener número par:", sprintf("%.4f%%", prob_par * 100), "\n")

# Tabla de probabilidades
tabla_urna <- data.frame(
  Numero = valores_urna,
  Tipo = ifelse(valores_urna %% 2 == 0, "Par", "Impar"),
  Probabilidad = prob_uniforme
)

kable(tabla_urna, 
      caption = "Distribución Uniforme Discreta - Urna con 8 bolas",
      align = "ccr",
      digits = 4) %>%
  kable_styling(bootstrap_options = "striped") %>%
  row_spec(which(tabla_urna$Tipo == "Par"), bold = TRUE, background = "#E3F2FD")

# Gráfico
plot_urna <- ggplot(tabla_urna, aes(x = Numero, y = Probabilidad, fill = Tipo)) +
  geom_col(color = "black", width = 0.7) +
  scale_fill_manual(values = c("Par" = "#4CAF50", "Impar" = "#BDC3C7")) +
  geom_hline(yintercept = prob_uniforme, linetype = "dashed", color = "red") +
  scale_x_continuous(breaks = valores_urna) +
  labs(
    title = "Distribución Uniforme Discreta - Urna",
    subtitle = paste("P(cada valor) =", prob_uniforme),
    x = "Número de bola",
    y = "Probabilidad"
  ) +
  theme_minimal() +
...
```{r}
cat("Número entero entre 5 y 15\n")

a <- 5
b <- 15
valores_enteros <- a:b
n_valores <- length(valores_enteros)

cat("Rango: [", a, ",", b, "]\n")
cat("Número de valores:", n_valores, "\n")

media_uniforme <- (a + b) / 2

varianza_uniforme <- ((b - a + 1)^2 - 1) / 12
desv_std_uniforme <- sqrt(varianza_uniforme)

cat("Parámetros estadísticos:\n")
cat("Media: μ = (a + b) / 2 =", media_uniforme, "\n")
cat("Varianza: σ² = ((b - a + 1)² - 1) / 12 =", round(varianza_uniforme, 4), "\n")
cat("Desviación estándar: σ =", round(desv_std_uniforme, 4), "\n")

media_calculada <- mean(valores_enteros)
varianza_calculada <- var(valores_enteros) * (n_valores - 1) / n_valores

cat("Verificación:\n")
cat("Media calculada:", media_calculada, "\n")
cat("Varianza calculada:", round(varianza_calculada, 4), "\n\n")

# Tabla de probabilidades
tabla_enteros <- data.frame(
  Valor = valores_enteros,
  Probabilidad = 1 / n_valores
)

kable(tabla_enteros, 
      caption = "Distribución Uniforme Discreta - Enteros de 5 a 15",
      align = "cr",
      digits = 4) %>%
  kable_styling(bootstrap_options = "striped")

# Gráfico
plot_enteros <- ggplot(tabla_enteros, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "#2196F3", alpha = 0.7, color = "black", width = 0.7) +
  geom_vline(xintercept = media_uniforme, linetype = "dashed", color = "red", linewidth = 1.2) +
  annotate("text", x = media_uniforme + 1, y = max(tabla_enteros$Probabilidad) * 0.9, 
           label = paste("μ =", media_uniforme), 
           color = "red", size = 5, fontface = "bold") +
  scale_x_continuous(breaks = valores_enteros) +
  labs(
    title = "Distribución Uniforme Discreta [5, 15]",
    subtitle = paste("n =", n_valores, "valores"),
    x = "Valor",
    y = "Probabilidad"
  ) +
  theme_minimal() +
...
```{r}
cat("Simulación de uniforme discreta\n")

valores_sim <- 0:4
n_valores_sim <- length(valores_sim)
n_simulaciones <- 500
set.seed(42)

cat("Valores posibles:", paste(valores_sim, collapse = ", "), "\n")
cat("Número de simulaciones:", n_simulaciones, "\n")

simulacion <- sample(valores_sim, n_simulaciones, replace = TRUE)

# Frecuencias observadas
frecuencias_obs <- table(factor(simulacion, levels = valores_sim))
prop_obs <- as.numeric(frecuencias_obs) / n_simulaciones

# Frecuencias teóricas
prob_teorica <- 1 / n_valores_sim
frecuencia_teorica <- n_simulaciones * prob_teorica

# Tabla comparativa
tabla_simulacion <- data.frame(
  Valor = valores_sim,
  Frec_Observada = as.numeric(frecuencias_obs),
  Frec_Teorica = frecuencia_teorica,
  Prop_Observada = prop_obs,
  Prop_Teorica = prob_teorica,
  Diferencia = abs(prop_obs - prob_teorica)
)

cat("Comparación de frecuencias:\n")
kable(tabla_simulacion, 
      caption = "Comparación: Frecuencias observadas vs teóricas",
      digits = 4,
      col.names = c("Valor", "Frec. Obs.", "Frec. Teór.", "Prop. Obs.", "Prop. Teór.", "Diferencia")) %>%
  kable_styling(bootstrap_options = "striped")

# Prueba de bondad de ajuste Chi-cuadrado
chi_test <- chisq.test(frecuencias_obs, p = rep(prob_teorica, n_valores_sim))

cat("\n\nPrueba de bondad de ajuste (Chi-cuadrado):\n")
cat("Estadístico χ²:", round(chi_test$statistic, 4), "\n")
cat("Grados de libertad:", chi_test$parameter, "\n")
cat("p-value:", round(chi_test$p.value, 4), "\n")
if (chi_test$p.value > 0.05) {
  cat("Conclusión: No se rechaza que la distribución sea uniforme (p > 0.05)\n\n")
} else {
  cat("Conclusión: Se rechaza la uniformidad (p < 0.05)\n\n")
}

# Estadísticas de la simulación
cat("Estadísticas de la simulación:\n")
cat("Media observada:", round(mean(simulacion), 4), "\n")
cat("Media teórica:", mean(valores_sim), "\n")
cat("Varianza observada:", round(var(simulacion), 4), "\n")
cat("Varianza teórica:", round(var(valores_sim) * (n_valores_sim - 1) / n_valores_sim, 4), "\n")

# Gráfico comparativo
df_comparacion <- data.frame(
  Valor = rep(valores_sim, 2),
  Proporcion = c(prop_obs, rep(prob_teorica, n_valores_sim)),
  Tipo = rep(c("Observada", "Teórica"), each = n_valores_sim)
)

plot_simulacion <- ggplot(df_comparacion, aes(x = Valor, y = Proporcion, fill = Tipo)) +
  geom_col(position = "dodge", alpha = 0.8, color = "black", width = 0.7) +
  scale_fill_manual(values = c("Observada" = "#FF9800", "Teórica" = "#9C27B0")) +
  geom_hline(yintercept = prob_teorica, linetype = "dashed", color = "red") +
  scale_x_continuous(breaks = valores_sim) +
  labs(
    title = "Simulación: Frecuencias observadas vs teóricas",
    subtitle = paste(n_simulaciones, "observaciones de Uniforme{0, 1, 2, 3, 4}"),
    x = "Valor",
    y = "Proporción",
    fill = "Tipo"
  ) +
