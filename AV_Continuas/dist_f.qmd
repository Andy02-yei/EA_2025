...
# a) Si X ~ t(n) entonces X² ~ F(1,n)

n <- 10
muestra_t <- rt(n_sim, df = n)
muestra_t2 <- muestra_t^2
muestra_f <- rf(n_sim, df1 = 1, df2 = n)

df_a <- data.frame(
  valor = c(muestra_t2, muestra_f),
  tipo = rep(c("X² (t² elevado)", "Y ~ F(1,10) directa"), each = n_sim)
) %>% filter(valor < 10)

x_vals <- seq(0, 10, length.out = 500)
pdf_teorica <- data.frame(x = x_vals, y = df(x_vals, df1 = 1, df2 = n))

ggplot() +
  geom_histogram(data = df_a, aes(x = valor, y = after_stat(density), fill = tipo),
                 alpha = 0.5, bins = 50, color = "black") +
  geom_line(data = pdf_teorica, aes(x = x, y = y), color = "red", linewidth = 1.2) +
  scale_fill_manual(values = c("#3498DB", "#2ECC71")) +
  labs(
    title = "a) Si X ~ t(n) entonces X² ~ F(1,n)",
    subtitle = paste("n =", n, "| Línea roja: PDF F(1,", n, ") teórica"),
    x = "Valor", y = "Densidad", fill = "Distribución"
  ) +
  theme_minimal() +
...
a <- 5; b <- 10
muestra_f1 <- rf(n_sim, df1 = a, df2 = b)
muestra_inv <- 1 / muestra_f1
muestra_f2 <- rf(n_sim, df1 = b, df2 = a)

df_b <- data.frame(
  valor = c(muestra_inv, muestra_f2),
  tipo = rep(c("1/X donde X ~ F(5,10)", "Y ~ F(10,5) directa"), each = n_sim)
) %>% filter(valor < 5)

x_vals <- seq(0, 5, length.out = 500)
pdf_teorica <- data.frame(x = x_vals, y = df(x_vals, df1 = b, df2 = a))

ggplot() +
  geom_histogram(data = df_b, aes(x = valor, y = after_stat(density), fill = tipo),
                 alpha = 0.5, bins = 50, color = "black") +
  geom_line(data = pdf_teorica, aes(x = x, y = y), color = "red", size = 1.2) +
  scale_fill_manual(values = c("#E74C3C", "#F39C12")) +
  labs(
    title = "b) Si X ~ F(a,b) entonces 1/X ~ F(b,a)",
    subtitle = paste("a =", a, ", b =", b, "| Línea roja: PDF F(", b, ",", a, ") teórica"),
    x = "Valor", y = "Densidad", fill = "Distribución"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 13),
...
a_chi <- 8; b_chi <- 12
X <- rchisq(n_sim, df = a_chi)
Y <- rchisq(n_sim, df = b_chi)
cociente <- (X / a_chi) / (Y / b_chi)
f_directa <- rf(n_sim, df1 = a_chi, df2 = b_chi)

df_c <- data.frame(
  valor = c(cociente, f_directa),
  tipo = rep(c("(X/a)/(Y/b) con χ²", "Z ~ F(8,12) directa"), each = n_sim)
) %>% filter(valor < 5)

x_vals <- seq(0, 5, length.out = 500)
pdf_teorica <- data.frame(x = x_vals, y = df(x_vals, df1 = a_chi, df2 = b_chi))

ggplot() +
  geom_histogram(data = df_c, aes(x = valor, y = after_stat(density), fill = tipo),
                 alpha = 0.5, bins = 50, color = "black") +
  geom_line(data = pdf_teorica, aes(x = x, y = y), color = "red", size = 1.2) +
  scale_fill_manual(values = c("#9B59B6", "#1ABC9C")) +
  labs(
    title = "c) Si (X/a)/(Y/b) con X,Y ~ χ² independientes → F(a,b)",
    subtitle = paste("a =", a_chi, ", b =", b_chi, "| Línea roja: PDF F(", a_chi, ",", b_chi, ") teórica"),
    x = "Valor", y = "Densidad", fill = "Distribución"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 13),
