...
cat("Distribución Beta(α =", alpha, ", β =", beta_param, ")\n")
...
cat("Media: μ =", round(media_teorica, 6), "\n")
cat("Varianza: σ² =", round(varianza_teorica, 6), "\n")
cat("Desviación estándar: σ =", round(desv_std_teorica, 6), "\n")

tamaños <- seq(100, 10000, by = 100)
n_tamaños <- length(tamaños)

medias_muestrales <- numeric(n_tamaños)
varianzas_muestrales <- numeric(n_tamaños)
muestra_completa <- rbeta(max(tamaños), alpha, beta_param)

# Calcular estadísticas para cada tamaño
for (i in seq_along(tamaños)) {
  n <- tamaños[i]
  muestra <- muestra_completa[1:n]
...
plot_media <- ggplot(df_convergencia, aes(x = n)) +
  geom_line(aes(y = media_muestral, color = "Media muestral"), linewidth = 1) +
  geom_hline(aes(yintercept = media_teorica, color = "Media teórica"), 
             linetype = "dashed", size = 1.2) +
  scale_color_manual(
    name = "",
    values = c("Media muestral" = "#2196F3", "Media teórica" = "#E74C3C")
  ) +
  labs(
...
plot_varianza <- ggplot(df_convergencia, aes(x = n)) +
  geom_line(aes(y = varianza_muestral, color = "Varianza muestral"), size = 1) +
  geom_hline(aes(yintercept = varianza_teorica, color = "Varianza teórica"), 
             linetype = "dashed", size = 1.2) +
  scale_color_manual(
    name = "",
    values = c("Varianza muestral" = "#4CAF50", "Varianza teórica" = "#FF9800")
  ) +
  labs(
...
muestra_final <- muestra_completa
media_final <- mean(muestra_final)
varianza_final <- var(muestra_final)

cat("\nEstadísticas de la muestra final (n = 10000):\n")
cat("Media muestral:", round(media_final, 6), "\n")
cat("Varianza muestral:", round(varianza_final, 6), "\n")
cat("Diferencia en media:", round(abs(media_final - media_teorica), 6), "\n")
cat("Diferencia en varianza:", round(abs(varianza_final - varianza_teorica), 6), "\n\n")

x_vals <- seq(0, 1, length.out = 1000)
y_teorico <- dbeta(x_vals, alpha, beta_param)

df_teorico <- data.frame(x = x_vals, y = y_teorico)
df_muestra <- data.frame(valor = muestra_final)
...
plot_histograma <- ggplot() +
  ...
  annotate("text", x = media_teorica + 0.1, y = max(y_teorico) * 0.9, 
           label = paste("μ =", round(media_teorica, 3)), 
           color = "royalblue4", size = 5, fontface = "bold") +
  labs(
    title = "Histograma de muestra Beta(2, 5) con PDF teórica",
    subtitle = paste("n = 10000 observaciones"),
    x = "Valor",
    y = "Densidad"
  ) +
  theme_minimal() +
...
print(plot_histograma)


#Hacemos una tabla comparativa

indices_tabla <- c(1, 10, 25, 50, 75, 100)
tabla_convergencia <- df_convergencia[indices_tabla, ] %>%
  mutate(
    Error_Media = abs(media_muestral - media_teorica),
    Error_Varianza = abs(varianza_muestral - varianza_teorica)
  ) %>%
  select(n, media_muestral, Error_Media, varianza_muestral, Error_Varianza)

kable(tabla_convergencia, 
      digits = 6,
      caption = "Convergencia de estadísticos muestrales a valores teóricos",
      col.names = c("n", "Media muestral", "Error media", "Varianza muestral", "Error varianza")) %>%
...
sigma_sq <- (alpha * beta_param) / ((alpha + beta_param)^2 * (alpha + beta_param + 1))
sigma <- sqrt(sigma_sq)

cat("Parámetros poblacionales:\n")
cat("μ =", round(mu, 6), "\n")
cat("σ² =", round(sigma_sq, 6), "\n")
cat("σ =", round(sigma, 6), "\n")

n_valores <- c(5, 10, 30, 50)
...
for (i in seq_along(n_valores)) {
  ...
  # Generar n_simulaciones medias muestrales
  medias_muestrales <- replicate(n_simulaciones, {
    muestra <- rbeta(n, alpha, beta_param)
    mean(muestra)
  })
  
  # Estandarizar según el TCL: Z = (X̄ₙ - μ) / (σ/√n)
  z_scores <- (medias_muestrales - mu) / (sigma / sqrt(n))
...
# Tabla
tabla_tcl <- data.frame(
  n = n_valores,
  `Error_Estandar` = sigma / sqrt(n_valores),
  `Media_Z` = sapply(resultados, function(x) x$media_z),
  `Desv_Z` = sapply(resultados, function(x) x$desv_z)
)

kable(tabla_tcl, 
      digits = 4,
      caption = "Verificación: (X̄ₙ - μ) / (σ/√n) → N(0,1)",
      col.names = c("n", "σ/√n", "Media Z", "Desv. Z")) %>%
  kable_styling(bootstrap_options = "striped")

cat("\n\nNota: Para que se cumpla el TCL, esperamos que:\n")
cat("- Media Z ≈ 0\n")
cat("- Desv. Z ≈ 1\n")
cat("- La convergencia mejora cuando n aumenta\n\n")
...
for (i in seq_along(n_valores)) {
  ...
  df_z <- data.frame(z = resultados[[i]]$z_scores)
  ...
  p <- ggplot(df_z, aes(x = z)) +
    ...
    labs(
      title = paste("n =", n),
      subtitle = paste("Z ~ N(", round(resultados[[i]]$media_z, 3), ", ", 
                       round(resultados[[i]]$desv_z^2, 3), ")", sep = ""),
      x = "Z = (X̄ₙ - μ) / (σ/√n)",
      y = "Densidad"
    ) +
    theme_minimal() +
...
# Combinar gráficos usando gridExtra
titulo <- textGrob(
  "Teorema Central del Límite: (X̄ₙ - μ) / (σ/√n) → N(0,1)\nDistribución Beta(2,5) | Línea roja: N(0,1) teórica",
  gp = gpar(fontsize = 14, fontface = "bold")
)

grid.arrange(grobs = plot_list, ncol = 2, top = titulo)
