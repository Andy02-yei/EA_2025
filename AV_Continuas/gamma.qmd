...
cat("Teorema Central del Límite - Distribución Gamma\n")
cat("Parámetros: α =", alpha, ", λ =", lambda, "\n")
...
cat("Parámetros teóricos de Gamma(", alpha, ", ", lambda, "):\n", sep = "")
cat("Media:", round(media_teorica, 4), "\n")
cat("Varianza:", round(varianza_teorica, 4), "\n")
cat("Desviación estándar:", round(desv_teorica, 4), "\n")

tamaños <- c(5, 10, 20, 30, 50, 100)
n_muestras <- 1000

cat("Tamaños de muestra a evaluar:", paste(tamaños, collapse = ", "), "\n")
cat("Número de muestras por tamaño:", n_muestras, "\n")
...
for (i in seq_along(tamaños)) {
  ...
  varianza_de_medias <- var(medias_muestrales)
  
  resultados[[i]] <- list(
    n = n,
    medias = medias_muestrales,
    media = media_de_medias,
    varianza = varianza_de_medias
  )
}
...
tabla_comparacion <- data.frame(
  ...
  Varianza_medias = sapply(resultados, function(x) x$varianza),
  Varianza_teorica = varianza_teorica / tamaños,
  Error_estandar = desv_teorica / sqrt(tamaños)
)

kable(tabla_comparacion, 
      digits = 4,
      caption = "Verificación del TCL para Gamma(2.5, 1.8)",
      col.names = c("n", "Media X̄", "Media teórica", "Var(X̄)", "σ²/n teórica", "σ/√n")) %>%
  kable_styling(bootstrap_options = "striped")

cat("- La media de las medias muestrales se aproxima a μ =", round(media_teorica, 4), "\n")
cat("- La varianza de las medias muestrales se aproxima a σ²/n\n")
cat("- El error disminuye conforme n aumenta (σ/√n)\n")
...
for (i in seq_along(tamaños)) {
  ...
  # Según el TCL: X̄ₙ ~ N(μ, σ²/n)
  media_tcl <- media_teorica
  desv_tcl <- desv_teorica / sqrt(n)
  
  df_medias <- data.frame(media = medias)
  
  # Rango para la curva normal
  x_range <- seq(min(medias), max(medias), length.out = 500)
  y_normal <- dnorm(x_range, mean = media_tcl, sd = desv_tcl)
  df_normal <- data.frame(x = x_range, y = y_normal)
  
  p <- ggplot() +
    geom_histogram(data = df_medias, aes(x = media, y = after_stat(density)),
                   bins = 30, fill = "#3498DB", alpha = 0.6, color = "black") +
    geom_line(data = df_normal, aes(x = x, y = y),
              color = "#E74C3C", linewidth = 1.3) +
    geom_vline(xintercept = media_teorica, linetype = "dashed", 
               color = "darkgreen", size = 1) +
    labs(
      title = paste("n =", n),
      subtitle = paste("X̄ ~ N(", round(media_tcl, 2), ", ", round(desv_tcl^2, 4), ")", sep = ""),
      x = "Media muestral",
      y = "Densidad"
    ) +
    theme_minimal() +
...
# Combinar gráficos (2 filas, 3 columnas)
combined <- grid.arrange(
  plot_list[[1]], plot_list[[2]], plot_list[[3]],
  plot_list[[4]], plot_list[[5]], plot_list[[6]],
  ncol = 3,
  top = "TCL: Distribución de medias muestrales de Gamma(2.5, 1.8)"
)

n_grande <- 100
idx_grande <- which(tamaños == n_grande)
medias_grande <- resultados[[idx_grande]]$medias

media_tcl_grande <- media_teorica
desv_tcl_grande <- desv_teorica / sqrt(n_grande)

df_grande <- data.frame(media = medias_grande)

x_range_grande <- seq(min(medias_grande) - 0.5, max(medias_grande) + 0.5, length.out = 1000)
y_normal_grande <- dnorm(x_range_grande, mean = media_tcl_grande, sd = desv_tcl_grande)
df_normal_grande <- data.frame(x = x_range_grande, y = y_normal_grande)

plot_grande <- ggplot() +
  geom_histogram(data = df_grande, aes(x = media, y = after_stat(density)),
                 bins = 40, fill = "#9B59B6", alpha = 0.6, color = "black") +
  geom_line(data = df_normal_grande, aes(x = x, y = y),
            color = "#E74C3C", size = 1.5) +
  geom_vline(xintercept = media_teorica, linetype = "dashed", 
             color = "darkgreen", size = 1.2) +
  annotate("text", x = media_teorica + 0.05, y = max(y_normal_grande) * 0.9,
           label = paste("μ =", round(media_teorica, 3)),
           color = "darkgreen", size = 5, fontface = "bold") +
  labs(
    title = "Convergencia del TCL con n = 100",
    x = "Media muestral X̄",
    y = "Densidad"
  ) +
  theme_minimal() +
...
plot_interactivo <- ggplotly(plot_grande)
print(plot_interactivo)

tabla_errores <- data.frame(
  n = tamaños,
  Error_media = abs(sapply(resultados, function(x) x$media) - media_teorica),
  Error_varianza = abs(sapply(resultados, function(x) x$varianza) - varianza_teorica / tamaños)
)

kable(tabla_errores, 
      digits = 6,
      caption = "Errores absolutos respecto a valores teóricos",
      col.names = c("n", "Error en media", "Error en varianza")) %>%
  kable_styling(bootstrap_options = "striped")
```
...
```{r}
cat("Verificación de la Proposición\n")
cat("Si X₁, ..., Xₙ ~ exp(λ) independientes,\n")
cat("entonces Y = ΣXᵢ ~ gamma(n, λ)\n")
...
simular_suma_exponenciales <- function(n, lambda, n_sim) {
  sumas <- replicate(n_sim, {
    exponenciales <- rexp(n, rate = lambda)
    sum(exponenciales)
  })
  
  # Generar muestra directa de Gamma
  gamma_directa <- rgamma(n_sim, shape = n, rate = lambda)
  
  media_suma <- mean(sumas)
  var_suma <- var(sumas)
  
  media_gamma <- mean(gamma_directa)
  var_gamma <- var(gamma_directa)
  
  # Valores teóricos
  media_teorica <- n / lambda
  var_teorica <- n / (lambda^2)
  
  return(list(
    n = n,
    sumas = sumas,
    gamma_directa = gamma_directa,
    media_suma = media_suma,
    var_suma = var_suma,
...
# Simulaciones para cada valor de n
resultados <- list()

for (i in seq_along(n_valores)) {
  n <- n_valores[i]
  resultados[[i]] <- simular_suma_exponenciales(n, lambda, n_simulaciones)
}

# Tabla comparativa
tabla_comparacion <- data.frame(
  n = n_valores,
  Media_suma = sapply(resultados, function(x) x$media_suma),
  Media_gamma = sapply(resultados, function(x) x$media_gamma),
  Media_teorica = sapply(resultados, function(x) x$media_teorica),
  Var_suma = sapply(resultados, function(x) x$var_suma),
  Var_gamma = sapply(resultados, function(x) x$var_gamma),
  Var_teorica = sapply(resultados, function(x) x$var_teorica)
)

kable(tabla_comparacion, 
      digits = 4,
      caption = "Verificación: Suma de exp(λ) vs Gamma(n, λ)",
      col.names = c("n", "Media Σexp", "Media Gamma", "Media teór.", 
                    "Var Σexp", "Var Gamma", "Var teór.")) %>%
  kable_styling(bootstrap_options = "striped")
...
for (i in seq_along(n_valores)) {
  n <- resultados[[i]]$n
  sumas <- resultados[[i]]$sumas
  gamma_directa <- resultados[[i]]$gamma_directa
  
  # Preparar datos
  df_comparacion <- data.frame(
    valor = c(sumas, gamma_directa),
    tipo = rep(c("Suma de exp(λ)", "Gamma(n, λ) directa"), each = n_simulaciones)
  )
  
  x_range <- seq(0, max(c(sumas, gamma_directa)) + 1, length.out = 500)
  y_teorico <- dgamma(x_range, shape = n, rate = lambda)
  df_teorico <- data.frame(x = x_range, y = y_teorico)
  
  p <- ggplot() +
    geom_histogram(data = df_comparacion, 
                   aes(x = valor, y = after_stat(density), fill = tipo),
                   alpha = 0.5, position = "identity", bins = 40, color = "black") +
    geom_line(data = df_teorico, aes(x = x, y = y),
              color = "#E74C3C", size = 1.5) +
    scale_fill_manual(values = c("Suma de exp(λ)" = "#3498DB", 
                                  "Gamma(n, λ) directa" = "#2ECC71")) +
    labs(
      title = paste("n =", n),
      x = "Valor",
      y = "Densidad",
...
# Combinar los 3 gráficos
combined <- grid.arrange(
  plot_list[[1]], plot_list[[2]], plot_list[[3]],
  ncol = 3,
  top = "Verificación: Suma de exponenciales ~ Gamma"
)

cat("\nAnálisis detallado para n = 5:\n")

n_5 <- resultados[[1]]$n
sumas_5 <- resultados[[1]]$sumas

cat("Estadísticas de la suma de 5 exponenciales:\n")
cat("Media:", round(resultados[[1]]$media_suma, 4), "\n")
cat("Varianza:", round(resultados[[1]]$var_suma, 4), "\n")
cat("Desv. Std.:", round(sd(sumas_5), 4), "\n")

cat("Valores teóricos de Gamma(5, 1.5):\n")
cat("Media: n/λ =", round(resultados[[1]]$media_teorica, 4), "\n")
cat("Varianza: n/λ² =", round(resultados[[1]]$var_teorica, 4), "\n")
cat("Desv. Std.:", round(sqrt(resultados[[1]]$var_teorica), 4), "\n")

# Gráfico detallado para n = 5
df_5 <- data.frame(
  valor = c(sumas_5, resultados[[1]]$gamma_directa),
  tipo = rep(c("Suma de 5 exp(1.5)", "Gamma(5, 1.5) directa"), each = n_simulaciones)
)

x_range_5 <- seq(0, max(df_5$valor) + 1, length.out = 1000)
y_teorico_5 <- dgamma(x_range_5, shape = 5, rate = lambda)
df_teorico_5 <- data.frame(x = x_range_5, y = y_teorico_5)

plot_5_detallado <- ggplot() +
  geom_histogram(data = df_5, 
                 aes(x = valor, y = after_stat(density), fill = tipo),
                 alpha = 0.6, position = "identity", bins = 50, color = "black") +
  geom_line(data = df_teorico_5, aes(x = x, y = y),
            color = "#E74C3C", size = 2) +
  scale_fill_manual(values = c("Suma de 5 exp(1.5)" = "#3498DB", 
                                "Gamma(5, 1.5) directa" = "#2ECC71")) +
  labs(
    title = "Verificación detallada: Suma de 5 exponenciales",
    x = "Valor",
    y = "Densidad",
    fill = "Distribución"
  ) +
...
plot_interactivo <- ggplotly(plot_5_detallado)
print(plot_interactivo)

x_max <- 20
x_comp <- seq(0, x_max, length.out = 1000)

df_formas <- data.frame(
  x = rep(x_comp, 3),
  densidad = c(dgamma(x_comp, shape = 5, rate = lambda),
               dgamma(x_comp, shape = 10, rate = lambda),
               dgamma(x_comp, shape = 20, rate = lambda)),
  n = factor(rep(c("n = 5", "n = 10", "n = 20"), each = length(x_comp)),
             levels = c("n = 5", "n = 10", "n = 20"))
)

plot_formas <- ggplot(df_formas, aes(x = x, y = densidad, color = n)) +
  geom_line(size = 1.5) +
  scale_color_manual(values = c("#E74C3C", "#3498DB", "#2ECC71")) +
  labs(
    title = "Evolución de la forma: Gamma(n, 1.5) para diferentes n",
    x = "Valor",
    y = "Densidad",
    color = "Parámetro"
  ) +
