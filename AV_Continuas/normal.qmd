...
mediciones_temp <- rnorm(n_mediciones, mean = mu_temp, sd = sigma_temp)

media_temp <- mean(mediciones_temp)
mediana_temp <- median(mediciones_temp)
sd_temp <- sd(mediciones_temp)

cat("Estadísticas de la muestra\n")
cat("Media de temperatura:", round(media_temp, 2), "°C\n")
cat("Mediana de temperatura:", round(mediana_temp, 2), "°C\n")
cat("Desviación estándar:", round(sd_temp, 2), "°C\n")
```

```{r}
# 2.
prob_fuera_inf <- pnorm(lim_inferior, mu_temp, sigma_temp)
prob_fuera_sup <- pnorm(lim_superior, mu_temp, sigma_temp, lower.tail = FALSE)
prob_fuera_total <- prob_fuera_inf + prob_fuera_sup

fuera_spec_muestra <- sum(mediciones_temp < lim_inferior | mediciones_temp > lim_superior)
porcentaje_fuera_muestra <- (fuera_spec_muestra / n_mediciones) * 100

cat("Análisis fuera de especificación\n")
cat("Probabilidad teórica fuera de spec:", round(prob_fuera_total * 100, 4), "%\n")
cat("Porcentaje observado fuera de spec:", round(porcentaje_fuera_muestra, 2), "%\n")
cat("Mediciones fuera de spec:", fuera_spec_muestra, "de", n_mediciones, "\n")
```

```{r}
# Visualización del proceso con límites de especificación
dist_temp <- tibble(
  temperatura = seq(mu_temp - 4*sigma_temp, mu_temp + 4*sigma_temp, length = 1000),
  pdf_temp = dnorm(temperatura, mean = mu_temp, sd = sigma_temp),
  cdf_temp = pnorm(temperatura, mean = mu_temp, sd = sigma_temp)
)

p1 <- ggplot(dist_temp) +
  geom_line(aes(temperatura, pdf_temp), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_temp |> filter(temperatura < lim_inferior | temperatura > lim_superior),
            aes(temperatura, pdf_temp), fill = "firebrick", alpha = 0.5) +
  geom_vline(xintercept = c(lim_inferior, lim_superior), linetype = "dashed", color = "red", linewidth = 1) +
  geom_vline(xintercept = mu_temp, linetype = "solid", color = "darkgreen", linewidth = 1) +
  annotate("label", x = 335, y = 0.05, 
           label = paste("Fuera de spec:\n", round(prob_fuera_total * 100, 3), "%"), 
           color = "white", size = 3.5, fill = "firebrick") +
  annotate("text", x = lim_inferior, y = 0.085, label = "340°C", hjust = -0.1, size = 3) +
  annotate("text", x = lim_superior, y = 0.085, label = "360°C", hjust = 1.1, size = 3) +
  labs(title = "Distribución de Temperatura (μ = 350°C, σ = 5°C)",
...
# Histograma de las mediciones con densidad teórica
p2 <- ggplot() +
  geom_histogram(aes(mediciones_temp, y = after_stat(density)), 
                 fill = "dodgerblue1", color = "dodgerblue3", bins = 30) +
  geom_line(data = dist_temp, aes(temperatura, pdf_temp), 
            color = "firebrick3", linewidth = 1.3) +
  geom_vline(xintercept = c(lim_inferior, lim_superior), 
             linetype = "dashed", color = "red", linewidth = 1) +
  labs(title = "Histograma de 1000 Mediciones",
       x = "Temperatura (°C)", y = "Densidad") +
  theme_bw()
...
# 3.
mu_descentrado <- 355
mediciones_desc <- rnorm(n_mediciones, mean = mu_descentrado, sd = sigma_temp)

prob_fuera_inf_desc <- pnorm(lim_inferior, mu_descentrado, sigma_temp)
prob_fuera_sup_desc <- pnorm(lim_superior, mu_descentrado, sigma_temp, lower.tail = FALSE)
prob_fuera_total_desc <- prob_fuera_inf_desc + prob_fuera_sup_desc

cat("Análisis con proceso descentrado\n")
cat("Probabilidad fuera de spec (centrado):", round(prob_fuera_total * 100, 4), "%\n")
cat("Probabilidad fuera de spec (descentrado):", round(prob_fuera_total_desc * 100, 4), "%\n")
cat("Incremento:", round((prob_fuera_total_desc - prob_fuera_total) * 100, 4), "puntos porcentuales\n")
```
```{r}
# Comparación visual: centrado vs descentrado
dist_temp_desc <- tibble(
  temperatura = seq(330, 370, length = 1000),
  pdf_centrado = dnorm(temperatura, mean = mu_temp, sd = sigma_temp),
  pdf_descentrado = dnorm(temperatura, mean = mu_descentrado, sd = sigma_temp)
)

p3 <- ggplot(dist_temp_desc) +
  geom_line(aes(temperatura, pdf_centrado, color = "Centrado (μ=350)"), linewidth = 1.2) +
  geom_line(aes(temperatura, pdf_descentrado, color = "Descentrado (μ=355)"), linewidth = 1.2) +
  geom_vline(xintercept = c(lim_inferior, lim_superior), 
             linetype = "dashed", color = "red", linewidth = 1) +
  geom_area(data = dist_temp_desc |> filter(temperatura < lim_inferior | temperatura > lim_superior),
            aes(temperatura, pdf_descentrado), fill = "orange", alpha = 0.3) +
  scale_color_manual(values = c("Centrado (μ=350)" = "#333AAA", 
                                  "Descentrado (μ=355)" = "darkorange")) +
  labs(title = "Comparación: Proceso Centrado vs Descentrado",
       x = "Temperatura (°C)", y = "Densidad", color = "Proceso") +
  theme_bw() +
...
```{r}
# 4.
z_requerido <- qnorm(0.9995)
sigma_necesario <- 10 / z_requerido

cat("Sigma necesario para 99.9% dentro de especificación\n")
cat("Valor z para 99.95% acumulado:", round(z_requerido, 4), "\n")
cat("Sigma actual:", sigma_temp, "°C\n")
cat("Sigma necesario:", round(sigma_necesario, 4), "°C\n")
cat("Reducción necesaria:", round(sigma_temp - sigma_necesario, 4), "°C\n")

# Verificación
prob_dentro_nuevo <- pnorm(lim_superior, mu_temp, sigma_necesario) - 
                     pnorm(lim_inferior, mu_temp, sigma_necesario)
cat("Verificación: Con σ =", round(sigma_necesario, 4), 
    "°C, el", round(prob_dentro_nuevo * 100, 2), "% está dentro de spec\n")
```

```{r}
# Comparación visual de diferentes sigmas
dist_comparacion <- tibble(
  temperatura = seq(330, 370, length = 1000),
  pdf_actual = dnorm(temperatura, mean = mu_temp, sd = sigma_temp),
  pdf_mejorado = dnorm(temperatura, mean = mu_temp, sd = sigma_necesario)
)

etiqueta_sigma_necesario <- paste0("σ = ", round(sigma_necesario, 2), "°C (necesario)")

p4 <- ggplot(dist_comparacion) +
  geom_line(aes(temperatura, pdf_actual, color = "σ = 5°C (actual)"), linewidth = 1.2) +
  geom_line(aes(temperatura, pdf_mejorado, color = etiqueta_sigma_necesario), linewidth = 1.2) +
  geom_vline(xintercept = c(lim_inferior, lim_superior), 
             linetype = "dashed", color = "red", linewidth = 1) +
  scale_color_manual(values = c("σ = 5°C (actual)" = "firebrick3", 
                                etiqueta_sigma_necesario = "darkgreen")) +
  labs(
    title = "Comparación: Sigma Actual vs Sigma Necesario para 99.9% Conformidad",
    x = "Temperatura (°C)", 
    y = "Densidad", 
    color = "Proceso"
  ) +
...
cat("Estadísticas de las ventas mensuales\n")
cat("Media de ventas: $", format(round(media_ventas, 2), big.mark = ","), "\n", sep = "")
cat("Mediana de ventas: $", format(round(mediana_ventas, 2), big.mark = ","), "\n", sep = "")
cat("Desviación estándar: $", format(round(sd_ventas, 2), big.mark = ","), "\n", sep = "")

tabla_ventas <- tibble(
  Mes = 1:n_meses,
  Ventas = paste0("$", format(round(ventas_mensuales, 2), big.mark = ","))
)

kable(tabla_ventas, 
      caption = "Ventas Mensuales Simuladas (24 meses)",
      align = "c") |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
```
...
prob_mayor_60k <- pnorm(umbral_alto, mu_ventas, sigma_ventas, lower.tail = FALSE)

meses_mayor_60k <- sum(ventas_mensuales > umbral_alto)
prob_muestra_60k <- meses_mayor_60k / n_meses

cat("Análisis de ventas superiores a $60,000\n")
cat("Probabilidad teórica: ", round(prob_mayor_60k * 100, 2), "%\n", sep = "")
cat("Porcentaje observado en muestra: ", round(prob_muestra_60k * 100, 2), "%\n", sep = "")
cat("Meses con ventas > $60,000: ", meses_mayor_60k, " de ", n_meses, "\n", sep = "")


# Visualización de la distribución con área > 60,000
dist_ventas <- tibble(
  ventas = seq(mu_ventas - 4*sigma_ventas, mu_ventas + 4*sigma_ventas, length = 1000),
  pdf_ventas = dnorm(ventas, mean = mu_ventas, sd = sigma_ventas),
  cdf_ventas = pnorm(ventas, mean = mu_ventas, sd = sigma_ventas)
)

p1 <- ggplot(dist_ventas) +
  geom_line(aes(ventas, pdf_ventas), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_ventas |> filter(ventas >= umbral_alto),
            aes(ventas, pdf_ventas), fill = "darkgreen", alpha = 0.5) +
  geom_vline(xintercept = umbral_alto, linetype = "dashed", color = "darkgreen", linewidth = 1) +
  geom_vline(xintercept = mu_ventas, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = 70000, y = 1.5e-05, 
           label = paste("P(X > 60,000) =\n", round(prob_mayor_60k * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "darkgreen") +
  scale_x_continuous(labels = scales::dollar_format(prefix = "$", big.mark = ",")) +
  labs(title = "Distribución de Ventas Mensuales (μ = $50,000, σ = $12,000)",
...
prob_menor_30k <- pnorm(umbral_bajo, mu_ventas, sigma_ventas)

# Número esperado de meses en 24 meses
meses_esperados_30k <- prob_menor_30k * n_meses

# En la muestra
meses_menor_30k <- sum(ventas_mensuales < umbral_bajo)

cat("Análisis de ventas inferiores a $30,000\n")
cat("Probabilidad teórica: ", round(prob_menor_30k * 100, 2), "%\n", sep = "")
cat("Meses esperados (en 24 meses): ", round(meses_esperados_30k, 2), "\n", sep = "")
cat("Meses observados en muestra: ", meses_menor_30k, " de ", n_meses, "\n", sep = "")

# Visualización con área < 30,000
p2 <- ggplot(dist_ventas) +
  geom_line(aes(ventas, pdf_ventas), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_ventas |> filter(ventas <= umbral_bajo),
            aes(ventas, pdf_ventas), fill = "firebrick", alpha = 0.5) +
  geom_vline(xintercept = umbral_bajo, linetype = "dashed", color = "firebrick", linewidth = 1) +
  geom_vline(xintercept = mu_ventas, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = 20000, y = 1.5e-05, 
           label = paste("P(X < 30,000) =\n", round(prob_menor_30k * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "firebrick") +
  scale_x_continuous(labels = scales::dollar_format(prefix = "$", big.mark = ",")) +
  labs(title = "Probabilidad de Ventas Menores a $30,000",
       x = "Ventas (pesos)", y = "Densidad") +
  theme_bw()
...
# 4.
cat("Ejercicio 4\n")
Q1_teorico <- qnorm(0.25, mu_ventas, sigma_ventas)
Q3_teorico <- qnorm(0.75, mu_ventas, sigma_ventas)
IQR_teorico <- Q3_teorico - Q1_teorico

# Muestral
Q1_muestra <- quantile(ventas_mensuales, 0.25)
Q3_muestra <- quantile(ventas_mensuales, 0.75)
IQR_muestra <- IQR(ventas_mensuales)

cat("Rango Intercuartílico\n")
cat("Valores teóricos\n")
cat("Q1 (Percentil 25): $", format(round(Q1_teorico, 2), big.mark = ","), "\n", sep = "")
cat("Q3 (Percentil 75): $", format(round(Q3_teorico, 2), big.mark = ","), "\n", sep = "")
cat("IQR teórico: $", format(round(IQR_teorico, 2), big.mark = ","), "\n", sep = "")

cat("Valores muestrales\n")
cat("Q1 (Percentil 25): $", format(round(Q1_muestra, 2), big.mark = ","), "\n", sep = "")
cat("Q3 (Percentil 75): $", format(round(Q3_muestra, 2), big.mark = ","), "\n", sep = "")
cat("IQR muestral: $", format(round(IQR_muestra, 2), big.mark = ","), "\n", sep = "")

# Visualización
p3 <- ggplot(dist_ventas) +
  geom_line(aes(ventas, pdf_ventas), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_ventas |> filter(ventas >= Q1_teorico & ventas <= Q3_teorico),
            aes(ventas, pdf_ventas), fill = "purple", alpha = 0.4) +
  geom_vline(xintercept = c(Q1_teorico, Q3_teorico), 
             linetype = "dashed", color = "purple", linewidth = 1) +
  geom_vline(xintercept = mu_ventas, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = mu_ventas, y = 2e-05, 
           label = paste("IQR = $", format(round(IQR_teorico, 0), big.mark = ","), sep = ""), 
           color = "white", size = 3.5, fill = "purple") +
  annotate("text", x = Q1_teorico, y = 2.5e-05, 
           label = "Q1", color = "purple", fontface = "bold", hjust = 1.2) +
  annotate("text", x = Q3_teorico, y = 2.5e-05, 
           label = "Q3", color = "purple", fontface = "bold", hjust = -0.2) +
  scale_x_continuous(labels = scales::dollar_format(prefix = "$", big.mark = ",")) +
  labs(title = "Rango Intercuartílico de las Ventas",
       subtitle = "El 50% central de las ventas está dentro del área sombreada",
       x = "Ventas (pesos)", y = "Densidad") +
  theme_bw()
...
prob_supera_meta <- pnorm(meta, mu_ventas, sigma_ventas, lower.tail = FALSE)

meses_supera_meta <- sum(ventas_mensuales > meta)
prob_muestra_meta <- meses_supera_meta / n_meses

cat("Cumplimiento de meta (ventas > $45,000)\n")
cat("Probabilidad teórica: ", round(prob_supera_meta * 100, 2), "%\n", sep = "")
cat("Porcentaje observado en muestra: ", round(prob_muestra_meta * 100, 2), "%\n", sep = "")
cat("Meses que cumplen meta: ", meses_supera_meta, " de ", n_meses, "\n", sep = "")

# Visualización
p4 <- ggplot(dist_ventas) +
  geom_line(aes(ventas, pdf_ventas), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_ventas |> filter(ventas >= meta),
            aes(ventas, pdf_ventas), fill = "gold3", alpha = 0.5) +
  geom_area(data = dist_ventas |> filter(ventas < meta),
            aes(ventas, pdf_ventas), fill = "gray50", alpha = 0.3) +
  geom_vline(xintercept = meta, linetype = "dashed", color = "gold4", linewidth = 1.2) +
  geom_vline(xintercept = mu_ventas, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = 65000, y = 1.5e-05, 
           label = paste("Cumple meta:\n", round(prob_supera_meta * 100, 2), "%"), 
           color = "black", size = 3.5, fill = "gold") +
  annotate("label", x = 30000, y = 1.5e-05, 
           label = paste("No cumple:\n", round((1-prob_supera_meta) * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "gray50") +
  scale_x_continuous(labels = scales::dollar_format(prefix = "$", big.mark = ",")) +
  labs(title = "Cumplimiento de Meta de Ventas ($45,000)",
       x = "Ventas (pesos)", y = "Densidad") +
  theme_bw()
...
media_vida <- mean(vida_baterias)
mediana_vida <- median(vida_baterias)
sd_vida <- sd(vida_baterias)
min_vida <- min(vida_baterias)
max_vida <- max(vida_baterias)

cat("Estadísticas de vida útil de baterías (muestra de 800)\n")
cat("Media de vida útil:", round(media_vida, 2), "horas\n")
cat("Mediana de vida útil:", round(mediana_vida, 2), "horas\n")
cat("Desviación estándar:", round(sd_vida, 2), "horas\n")
cat("Mínimo:", round(min_vida, 2), "horas\n")
cat("Máximo:", round(max_vida, 2), "horas\n")
```
```{r}
# 2.
cat("Ejercicio 2\n")
garantia_95 <- qnorm(0.05, mu_bateria, sigma_bateria)

cat("Garantía que cubre al 95% de las baterías\n")
cat("Percentil 5 (garantía recomendada):", round(garantia_95, 2), "horas\n")
cat("Interpretación: El 95% de las baterías durarán más de", round(garantia_95, 2), "horas\n")
cat("Solo el 5% fallará antes de este tiempo\n")

baterias_bajo_garantia <- sum(vida_baterias < garantia_95)
porcentaje_bajo_garantia <- (baterias_bajo_garantia / n_baterias) * 100

cat("Baterías que fallan antes de", round(garantia_95, 2), "horas:", 
    baterias_bajo_garantia, "de", n_baterias, 
    "(", round(porcentaje_bajo_garantia, 2), "%)\n")

# Visualización de la garantía
dist_bateria <- tibble(
  horas = seq(mu_bateria - 4*sigma_bateria, mu_bateria + 4*sigma_bateria, length = 1000),
  pdf_bateria = dnorm(horas, mean = mu_bateria, sd = sigma_bateria),
  cdf_bateria = pnorm(horas, mean = mu_bateria, sd = sigma_bateria)
)

p1 <- ggplot(dist_bateria) +
  geom_line(aes(horas, pdf_bateria), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_bateria |> filter(horas >= garantia_95),
            aes(horas, pdf_bateria), fill = "darkgreen", alpha = 0.5) +
  geom_area(data = dist_bateria |> filter(horas < garantia_95),
            aes(horas, pdf_bateria), fill = "firebrick", alpha = 0.5) +
  geom_vline(xintercept = garantia_95, linetype = "dashed", color = "red", linewidth = 1.2) +
  geom_vline(xintercept = mu_bateria, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = 1300, y = 0.0015, 
           label = paste("95% duran\nmás de", round(garantia_95, 0), "hrs"), 
           color = "white", size = 3.5, fill = "darkgreen") +
  annotate("label", x = 850, y = 0.0015, 
           label = "5% fallan\nantes", 
           color = "white", size = 3.5, fill = "firebrick") +
  annotate("text", x = garantia_95, y = 0.0028, 
           label = paste("Garantía:", round(garantia_95, 0), "hrs"), 
           angle = 90, vjust = -0.5, color = "red", fontface = "bold") +
  labs(title = "Distribución de Vida Útil de Baterías (μ = 1200 hrs, σ = 150 hrs)",
       subtitle = "Garantía que cubre al 95% de las baterías",
       x = "Vida útil (horas)", y = "Densidad") +
  theme_bw()
...
cat("Ejercicio 3\n")
n_vendidas <- 10000
garantia_ofrecida <- 900

# Probabilidad de falla antes de 900 horas
prob_falla <- pnorm(garantia_ofrecida, mu_bateria, sigma_bateria)

baterias_fallan <- prob_falla * n_vendidas

cat("Análisis de fallas con garantía de 900 horas\n")
cat("Garantía ofrecida:", garantia_ofrecida, "horas\n")
cat("Probabilidad de falla antes de", garantia_ofrecida, "horas:", 
    round(prob_falla * 100, 4), "%\n")
cat("Baterías vendidas:", format(n_vendidas, big.mark = ","), "\n")
cat("Baterías esperadas que fallen:", round(baterias_fallan, 0), "\n")
cat("Esto representa el", round(prob_falla * 100, 2), "% de las ventas\n\n")

# Ejemplo de costos estimados
cat("Análisis de impacto económico (ejemplo)\n")
costo_reemplazo <- 500  # pesos por batería
costo_total_garantias <- baterias_fallan * costo_reemplazo
cat("Si cada reemplazo cuesta $", costo_reemplazo, ":\n", sep = "")
cat("Costo total estimado por garantías: $", format(round(costo_total_garantias, 0), big.mark = ","), "\n", sep = "")

p2 <- ggplot(dist_bateria) +
  geom_line(aes(horas, pdf_bateria), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_bateria |> filter(horas < garantia_ofrecida),
            aes(horas, pdf_bateria), fill = "firebrick", alpha = 0.6) +
  geom_area(data = dist_bateria |> filter(horas >= garantia_ofrecida),
            aes(horas, pdf_bateria), fill = "darkgreen", alpha = 0.4) +
  geom_vline(xintercept = garantia_ofrecida, linetype = "dashed", 
             color = "red", linewidth = 1.2) +
  geom_vline(xintercept = mu_bateria, linetype = "solid", 
             color = "blue", linewidth = 0.8) +
  annotate("label", x = 700, y = 0.0018, 
           label = paste("Fallan:", round(baterias_fallan, 0), "baterías\n",
                        round(prob_falla * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "firebrick") +
  annotate("text", x = garantia_ofrecida, y = 0.0028, 
           label = "900 hrs", 
           angle = 90, vjust = -0.5, color = "red", fontface = "bold") +
  labs(title = "Impacto de Garantía de 900 Horas",
       subtitle = paste("De", format(n_vendidas, big.mark = ","), 
                       "baterías, se espera que", round(baterias_fallan, 0), "fallen"),
       x = "Vida útil (horas)", y = "Densidad") +
  theme_bw()
...
# 4.
cat("Ejercicio 4\n")
media_teorica <- mu_bateria
mediana_teorica <- mu_bateria

# Diferencias
diff_media <- media_vida - media_teorica
diff_mediana <- mediana_vida - mediana_teorica

cat("Comparación: Valores muestrales vs teóricos\n")
cat("Media\n")
cat("Teórica:", media_teorica, "horas\n")
cat("Muestral:", round(media_vida, 2), "horas\n")
cat("Diferencia:", round(diff_media, 2), "horas\n")
cat("Error relativo:", round((diff_media / media_teorica) * 100, 3), "%\n\n")

cat("Mediana\n")
cat("Teórica:", mediana_teorica, "horas\n")
cat("Muestral:", round(mediana_vida, 2), "horas\n")
cat("Diferencia:", round(diff_mediana, 2), "horas\n")
cat("Error relativo:", round((diff_mediana / mediana_teorica) * 100, 3), "%\n\n")

cat("Desviación estándar\n")
cat("Teórica:", sigma_bateria, "horas\n")
cat("Muestral:", round(sd_vida, 2), "horas\n")
cat("Diferencia:", round(sd_vida - sigma_bateria, 2), "horas\n")
```
```{r}
# 5.
cat("Ejercicio 5\n")
percentil_10_teorico <- qnorm(0.10, mu_bateria, sigma_bateria)

percentil_10_muestra <- quantile(vida_baterias, 0.10)

cat("Percentil 10 de la distribución\n")
cat("Percentil 10 teórico:", round(percentil_10_teorico, 2), "horas\n")
cat("Percentil 10 muestral:", round(percentil_10_muestra, 2), "horas\n")

p3 <- ggplot(dist_bateria) +
  geom_line(aes(horas, pdf_bateria), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_bateria |> filter(horas <= percentil_10_teorico),
            aes(horas, pdf_bateria), fill = "orange", alpha = 0.6) +
  geom_vline(xintercept = percentil_10_teorico, linetype = "dashed", 
             color = "darkorange", linewidth = 1.2) +
  geom_vline(xintercept = mu_bateria, linetype = "solid", 
             color = "blue", linewidth = 0.8) +
  annotate("label", x = 950, y = 0.0015, 
           label = "10% duran\nmenos de esto", 
           color = "white", size = 3.5, fill = "darkorange") +
  annotate("text", x = percentil_10_teorico, y = 0.0028, 
           label = paste("P10 =", round(percentil_10_teorico, 0), "hrs"), 
           angle = 90, vjust = -0.5, color = "darkorange", fontface = "bold") +
  labs(title = "Percentil 10 de la Vida Útil de Baterías",
       subtitle = "El 10% de las baterías fallan antes de este punto",
       x = "Vida útil (horas)", y = "Densidad") +
  theme_bw()
...
media_error <- mean(errores)
mediana_error <- median(errores)
sd_error <- sd(errores)

cat("Estadísticas de errores de medición\n")
cat("Media de errores:", round(media_error, 4), "unidades\n")
cat("Mediana de errores:", round(mediana_error, 4), "unidades\n")
cat("Desviación estándar:", round(sd_error, 4), "unidades\n")
```
...
prob_entre_5 <- pnorm(lim_superior, mu_error, sigma_error) - 
                pnorm(lim_inferior, mu_error, sigma_error)

errores_entre_5 <- sum(errores >= lim_inferior & errores <= lim_superior)
porcentaje_muestra <- (errores_entre_5 / n_mediciones) * 100

cat("Errores entre -5 y +5 unidades\n")
cat("Probabilidad teórica:", round(prob_entre_5 * 100, 2), "%\n")
cat("Porcentaje observado:", round(porcentaje_muestra, 2), "%\n")
cat("Mediciones en rango:", errores_entre_5, "de", n_mediciones, "\n")

dist_error <- tibble(
  error = seq(mu_error - 4*sigma_error, mu_error + 4*sigma_error, length = 1000),
  pdf_error = dnorm(error, mean = mu_error, sd = sigma_error),
  cdf_error = pnorm(error, mean = mu_error, sd = sigma_error)
)

p1 <- ggplot(dist_error) +
  geom_line(aes(error, pdf_error), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_error |> filter(error >= lim_inferior & error <= lim_superior),
            aes(error, pdf_error), fill = "darkgreen", alpha = 0.5) +
  geom_area(data = dist_error |> filter(error < lim_inferior | error > lim_superior),
            aes(error, pdf_error), fill = "firebrick", alpha = 0.3) +
  geom_vline(xintercept = c(lim_inferior, lim_superior), 
             linetype = "dashed", color = "darkgreen", linewidth = 1) +
  geom_vline(xintercept = 0, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = 0, y = 0.12, 
           label = paste("Entre -5 y +5:\n", round(prob_entre_5 * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "darkgreen") +
  labs(title = "Distribución de Errores de Medición N(0, 2.5²)",
...
# P(|X| > 6) = P(X < -6) + P(X > 6)
prob_menor_neg6 <- pnorm(-umbral_abs, mu_error, sigma_error)
prob_mayor_6 <- pnorm(umbral_abs, mu_error, sigma_error, lower.tail = FALSE)
prob_abs_mayor_6 <- prob_menor_neg6 + prob_mayor_6

errores_abs_mayor_6 <- sum(abs(errores) > umbral_abs)
porcentaje_abs_muestra <- (errores_abs_mayor_6 / n_mediciones) * 100

cat("Probabilidad de |error| > 6 unidades\n")
cat("P(error < -6):", round(prob_menor_neg6 * 100, 4), "%\n")
cat("P(error > +6):", round(prob_mayor_6 * 100, 4), "%\n")
cat("P(|error| > 6):", round(prob_abs_mayor_6 * 100, 4), "%\n")
cat("Porcentaje observado:", round(porcentaje_abs_muestra, 2), "%\n")
cat("Mediciones con |error| > 6:", errores_abs_mayor_6, "de", n_mediciones, "\n")


p2 <- ggplot(dist_error) +
  geom_line(aes(error, pdf_error), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_error |> filter(error < -umbral_abs | error > umbral_abs),
            aes(error, pdf_error), fill = "firebrick", alpha = 0.6) +
  geom_vline(xintercept = c(-umbral_abs, umbral_abs), 
             linetype = "dashed", color = "red", linewidth = 1) +
  geom_vline(xintercept = 0, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = -8, y = 0.10, 
           label = paste("P(|error| > 6) =\n", round(prob_abs_mayor_6 * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "firebrick") +
  annotate("text", x = -umbral_abs, y = 0.17, label = "-6", color = "red", fontface = "bold") +
  annotate("text", x = umbral_abs, y = 0.17, label = "+6", color = "red", fontface = "bold") +
  labs(title = "Errores Mayores a 6 Unidades en Valor Absoluto",
       x = "Error (unidades)", y = "Densidad") +
  theme_bw()
...
# P(|X| ≤ 3) = P(-3 ≤ X ≤ 3)
prob_preciso <- pnorm(umbral_precision, mu_error, sigma_error) - 
                pnorm(-umbral_precision, mu_error, sigma_error)

mediciones_precisas <- sum(abs(errores) <= umbral_precision)
porcentaje_precisas <- (mediciones_precisas / n_mediciones) * 100

cat("Mediciones precisas (|error| ≤ 3 unidades)\n")
cat("Probabilidad teórica:", round(prob_preciso * 100, 2), "%\n")
cat("Porcentaje observado:", round(porcentaje_precisas, 2), "%\n")
cat("Mediciones precisas:", mediciones_precisas, "de", n_mediciones, "\n")

p3 <- ggplot(dist_error) +
  geom_line(aes(error, pdf_error), color = "#333AAA", linewidth = 1.3) +
  geom_area(data = dist_error |> filter(error >= -umbral_precision & error <= umbral_precision),
            aes(error, pdf_error), fill = "gold3", alpha = 0.6) +
  geom_area(data = dist_error |> filter(error < -umbral_precision | error > umbral_precision),
            aes(error, pdf_error), fill = "gray50", alpha = 0.3) +
  geom_vline(xintercept = c(-umbral_precision, umbral_precision), 
             linetype = "dashed", color = "gold4", linewidth = 1) +
  geom_vline(xintercept = 0, linetype = "solid", color = "blue", linewidth = 0.8) +
  annotate("label", x = 0, y = 0.12, 
           label = paste("Precisas:\n", round(prob_preciso * 100, 2), "%"), 
           color = "black", size = 3.5, fill = "gold") +
  annotate("text", x = -umbral_precision, y = 0.17, label = "-3", 
           color = "gold4", fontface = "bold") +
  annotate("text", x = umbral_precision, y = 0.17, label = "+3", 
           color = "gold4", fontface = "bold") +
  labs(title = "Mediciones Precisas (|error| ≤ 3 unidades)",
       x = "Error (unidades)", y = "Densidad") +
  theme_bw()
...
# 5.
cat("Ejercicio 5\n")
errores_negativos <- sum(errores < 0)
errores_positivos <- sum(errores > 0)
errores_cero <- sum(errores == 0)

cat("Verificación de simetría alrededor de 0\n")
cat("Media teórica (centro):", mu_error, "unidades\n")
cat("Media muestral:", round(media_error, 4), "unidades\n")
cat("Mediana muestral:", round(mediana_error, 4), "unidades\n\n")

cat("Distribución de errores:\n")
cat("Errores negativos:", errores_negativos, "(", round(errores_negativos/n_mediciones*100, 2), "%)\n")
cat("Errores positivos:", errores_positivos, "(", round(errores_positivos/n_mediciones*100, 2), "%)\n")
cat("Errores = 0:", errores_cero, "\n\n")

asimetria <- skewness(errores)
cat("Coeficiente de asimetría:", round(asimetria, 4), "\n")
cat("Interpretación: Cercano a 0 indica simetría\n\n")

percentiles_comp <- tibble(
  Percentil = c("P10 vs P90", "P25 vs P75", "P5 vs P95"),
  `Valor Inferior` = c(
    round(quantile(errores, 0.10), 3),
    round(quantile(errores, 0.25), 3),
    round(quantile(errores, 0.05), 3)
  ),
  `Valor Superior` = c(
    round(quantile(errores, 0.90), 3),
    round(quantile(errores, 0.75), 3),
    round(quantile(errores, 0.95), 3)
  ),
  `Suma (debe ≈ 0)` = c(
    round(quantile(errores, 0.10) + quantile(errores, 0.90), 3),
    round(quantile(errores, 0.25) + quantile(errores, 0.75), 3),
    round(quantile(errores, 0.05) + quantile(errores, 0.95), 3)
  )
)

kable(percentiles_comp, 
      caption = "Verificación de Simetría mediante Percentiles",
      align = "c") |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)

p4 <- ggplot() +
  geom_histogram(aes(errores, y = after_stat(density)), 
                 fill = "dodgerblue1", color = "dodgerblue3", bins = 30) +
  geom_line(data = dist_error, aes(error, pdf_error), 
            color = "firebrick3", linewidth = 1.3) +
  geom_vline(xintercept = 0, linetype = "solid", color = "darkgreen", linewidth = 1.2) +
  geom_vline(xintercept = media_error, linetype = "dashed", color = "blue", linewidth = 1) +
  annotate("text", x = 0.5, y = 0.16, label = "μ = 0", 
           color = "darkgreen", fontface = "bold", hjust = 0) +
  labs(title = "Histograma de Errores: Verificación de Simetría",
       subtitle = "La distribución debe ser simétrica alrededor de 0",
       x = "Error (unidades)", y = "Densidad") +
  theme_bw()
...
cat("Ejercicio 1\n\n")
set.seed(147)
puntuaciones_A <- rnorm(n_estudiantes, mean = mu_A, sd = sigma_A)
puntuaciones_B <- rnorm(n_estudiantes, mean = mu_B, sd = sigma_B)

cat("Estadísticas Grupo A\n")
cat("Media:", round(mean(puntuaciones_A), 2), "puntos\n")
cat("Mediana:", round(median(puntuaciones_A), 2), "puntos\n")
cat("Desviación estándar:", round(sd(puntuaciones_A), 2), "puntos\n")
cat("Mínimo:", round(min(puntuaciones_A), 2), "puntos\n")
cat("Máximo:", round(max(puntuaciones_A), 2), "puntos\n\n")

cat("Estadísticas Grupo B\n")
cat("Media:", round(mean(puntuaciones_B), 2), "puntos\n")
cat("Mediana:", round(median(puntuaciones_B), 2), "puntos\n")
cat("Desviación estándar:", round(sd(puntuaciones_B), 2), "puntos\n")
cat("Mínimo:", round(min(puntuaciones_B), 2), "puntos\n")
cat("Máximo:", round(max(puntuaciones_B), 2), "puntos\n")
```

```{r}
# 2.
cat("Ejercicio 2\n\n")
prob_A_supera_media_B <- pnorm(mu_B, mu_A, sigma_A, lower.tail = FALSE)

estudiantes_A_supera <- sum(puntuaciones_A > mu_B)
porcentaje_A_supera <- (estudiantes_A_supera / n_estudiantes) * 100

cat("Estudiantes del Grupo A que superan la media del Grupo B (82)\n")
cat("Media del Grupo B:", mu_B, "puntos\n")
cat("Probabilidad teórica (Grupo A):", round(prob_A_supera_media_B * 100, 2), "%\n")
cat("Porcentaje observado:", round(porcentaje_A_supera, 2), "%\n")
cat("Estudiantes del Grupo A que superan 82:", estudiantes_A_supera, "de", n_estudiantes, "\n\n")

dist_A <- tibble(
  puntos = seq(mu_A - 4*sigma_A, mu_A + 4*sigma_A, length = 1000),
  pdf_A = dnorm(puntos, mean = mu_A, sd = sigma_A)
)

p1 <- ggplot(dist_A) +
  geom_line(aes(puntos, pdf_A), color = "#D55E00", linewidth = 1.3) +
  geom_area(data = dist_A |> filter(puntos >= mu_B),
            aes(puntos, pdf_A), fill = "#D55E00", alpha = 0.5) +
  geom_vline(xintercept = mu_B, linetype = "dashed", color = "#0072B2", linewidth = 1.2) +
  geom_vline(xintercept = mu_A, linetype = "solid", color = "#D55E00", linewidth = 0.8) +
  annotate("label", x = 90, y = 0.025, 
           label = paste("Grupo A > 82:\n", round(prob_A_supera_media_B * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "#D55E00") +
  annotate("text", x = mu_B, y = 0.042, 
           label = "Media Grupo B = 82", 
           angle = 90, vjust = -0.5, color = "#0072B2", fontface = "bold") +
  labs(title = "Grupo A: ¿Cuántos superan la media del Grupo B?",
...
prob_B_menor_70 <- pnorm(umbral, mu_B, sigma_B)

estudiantes_B_menor_70 <- sum(puntuaciones_B < umbral)
porcentaje_B_menor_70 <- (estudiantes_B_menor_70 / n_estudiantes) * 100

cat("Estudiantes del Grupo B con puntuación < 70\n")
cat("Probabilidad teórica:", round(prob_B_menor_70 * 100, 4), "%\n")
cat("Porcentaje observado:", round(porcentaje_B_menor_70, 2), "%\n")
cat("Estudiantes del Grupo B < 70:", estudiantes_B_menor_70, "de", n_estudiantes, "\n")

dist_B <- tibble(
  puntos = seq(mu_B - 4*sigma_B, mu_B + 4*sigma_B, length = 1000),
  pdf_B = dnorm(puntos, mean = mu_B, sd = sigma_B)
)

p2 <- ggplot(dist_B) +
  geom_line(aes(puntos, pdf_B), color = "#0072B2", linewidth = 1.3) +
  geom_area(data = dist_B |> filter(puntos < umbral),
            aes(puntos, pdf_B), fill = "#0072B2", alpha = 0.5) +
  geom_vline(xintercept = umbral, linetype = "dashed", color = "red", linewidth = 1.2) +
  geom_vline(xintercept = mu_B, linetype = "solid", color = "#0072B2", linewidth = 0.8) +
  annotate("label", x = 62, y = 0.03, 
           label = paste("P(B < 70) =\n", round(prob_B_menor_70 * 100, 2), "%"), 
           color = "white", size = 3.5, fill = "#0072B2") +
  annotate("text", x = umbral, y = 0.052, 
           label = "70 puntos", 
           angle = 90, vjust = -0.5, color = "red", fontface = "bold") +
  labs(title = "Grupo B: Probabilidad de puntuación menor a 70",
       x = "Puntuación", y = "Densidad") +
  theme_bw()
...
cat("Ejercicio4\n")
dist_comparacion <- tibble(
  puntos = seq(40, 110, length = 1000),
  pdf_A = dnorm(puntos, mean = mu_A, sd = sigma_A),
  pdf_B = dnorm(puntos, mean = mu_B, sd = sigma_B)
)

p3 <- ggplot(dist_comparacion) +
  geom_line(aes(puntos, pdf_A, color = "Grupo A"), linewidth = 1.3) +
  geom_line(aes(puntos, pdf_B, color = "Grupo B"), linewidth = 1.3) +
  geom_vline(xintercept = mu_A, linetype = "dashed", color = "#D55E00", linewidth = 0.8) +
  geom_vline(xintercept = mu_B, linetype = "dashed", color = "#0072B2", linewidth = 0.8) +
  scale_color_manual(values = c("Grupo A" = "#D55E00", "Grupo B" = "#0072B2")) +
  annotate("text", x = mu_A, y = 0.045, label = "μ_A = 75", 
           color = "#D55E00", fontface = "bold", hjust = 1.1) +
  annotate("text", x = mu_B, y = 0.052, label = "μ_B = 82", 
           color = "#0072B2", fontface = "bold", hjust = -0.1) +
  labs(title = "Comparación de Distribuciones: Grupo A vs Grupo B",
       x = "Puntuación", y = "Densidad", color = "") +
  theme_bw() +
  theme(legend.position = "top")
...
datos_combinados <- tibble(
  ...
  Grupo = rep(c("Grupo A", "Grupo B"), each = n_estudiantes)
)

p4 <- ggplot(datos_combinados, aes(x = Puntuacion, fill = Grupo)) +
  geom_histogram(aes(y = after_stat(density)), alpha = 0.6, position = "identity", bins = 25) +
  geom_density(aes(color = Grupo), linewidth = 1.2, alpha = 0) +
  scale_fill_manual(values = c("Grupo A" = "#D55E00", "Grupo B" = "#0072B2")) +
  scale_color_manual(values = c("Grupo A" = "#D55E00", "Grupo B" = "#0072B2")) +
  labs(title = "Histogramas Superpuestos: Grupo A vs Grupo B",
       x = "Puntuación", y = "Densidad") +
  theme_bw() +
  theme(legend.position = "top")
...
# 5.
cat("Ejercicio 5\n")
percentil_90_A_teorico <- qnorm(0.90, mu_A, sigma_A)
percentil_90_B_teorico <- qnorm(0.90, mu_B, sigma_B)

percentil_90_A_muestra <- quantile(puntuaciones_A, 0.90)
percentil_90_B_muestra <- quantile(puntuaciones_B, 0.90)

cat("Percentil 90 (10% superior de cada grupo)\n\n")
cat("Grupo A\n")
cat("Percentil 90 teórico:", round(percentil_90_A_teorico, 2), "puntos\n")
cat("Percentil 90 muestral:", round(percentil_90_A_muestra, 2), "puntos\n")
cat("Interpretación: El 10% superior del Grupo A tiene ≥", 
    round(percentil_90_A_teorico, 2), "puntos\n\n")

cat("Grupo B\n")
cat("Percentil 90 teórico:", round(percentil_90_B_teorico, 2), "puntos\n")
cat("Percentil 90 muestral:", round(percentil_90_B_muestra, 2), "puntos\n")
cat("Interpretación: El 10% superior del Grupo B tiene ≥", 
    round(percentil_90_B_teorico, 2), "puntos\n\n")

cat("Diferencia entre grupos (P90):", 
    round(percentil_90_B_teorico - percentil_90_A_teorico, 2), "puntos\n\n")


p6 <- ggplot(dist_comparacion) +
  geom_line(aes(puntos, pdf_A, color = "Grupo A"), linewidth = 1.3) +
  geom_line(aes(puntos, pdf_B, color = "Grupo B"), linewidth = 1.3) +
  geom_area(data = dist_comparacion |> filter(puntos >= percentil_90_A_teorico),
            aes(puntos, pdf_A), fill = "#D55E00", alpha = 0.4) +
  geom_area(data = dist_comparacion |> filter(puntos >= percentil_90_B_teorico),
            aes(puntos, pdf_B), fill = "#0072B2", alpha = 0.4) +
  geom_vline(xintercept = percentil_90_A_teorico, linetype = "dashed", 
             color = "#D55E00", linewidth = 1) +
  geom_vline(xintercept = percentil_90_B_teorico, linetype = "dashed", 
             color = "#0072B2", linewidth = 1) +
  scale_color_manual(values = c("Grupo A" = "#D55E00", "Grupo B" = "#0072B2")) +
  annotate("text", x = percentil_90_A_teorico, y = 0.045, 
           label = paste("P90_A =", round(percentil_90_A_teorico, 1)), 
           angle = 90, vjust = -0.5, color = "#D55E00", fontface = "bold", size = 3) +
  annotate("text", x = percentil_90_B_teorico, y = 0.052, 
           label = paste("P90_B =", round(percentil_90_B_teorico, 1)), 
           angle = 90, vjust = -0.5, color = "#0072B2", fontface = "bold", size = 3) +
  labs(title = "10% Superior de Cada Grupo (Percentil 90)",
       subtitle = "Áreas sombreadas representan el 10% con mayores puntuaciones",
       x = "Puntuación", y = "Densidad", color = "") +
  theme_bw() +
  theme(legend.position = "top")
